...
All public functions:
  call_gemini(prompt)                          -> (text, source)
  call_huggingface(prompt)                     -> (text, source)
  call_llm(prompt, max_tokens)                 -> (text, source)  [compatibility alias]
  offline_brief(ticker, ...)                   -> str
  generate_resilient_brief(prompt, ticker, ...) -> str
"""
import os
import requests
...
def call_gemini(prompt: str):
    """Returns (text, source_label). Never raises."""
...
# ── call_llm: compatibility alias used by older agents ──────────────────
def call_llm(prompt: str, max_tokens: int = 300):
    """
    Compatibility wrapper so any agent using `call_llm` keeps working.
    Tries Gemini first, then HuggingFace, returns (text, source).
    Returns (None, reason) if both fail — caller should handle.
    """
    text, src = call_gemini(prompt)
    if text:
        return text, src
    text, src2 = call_huggingface(prompt)
    if text:
        return text, src2
    return None, f"All LLMs failed ({src} | {src2})"
...
---
*⚙️ Generated by StockSense AI offline engine · Rule-based fallback*
*⚠️ For educational purposes only. Not SEBI-registered investment advice.*""".strip()


# ── Main resilient brief (called by ExplanationAgent) ───────────────────
def generate_resilient_brief(
    ...
    if text and len(text) > 80:
        return text + f"\n\n---\n*🤖 HuggingFace Mistral (Gemini: {gemini_fail})*"

    return offline_brief(ticker, pattern_data, signal_data, sentiment_data)
