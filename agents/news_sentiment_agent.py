...
def analyze_news_sentiment(ticker: str) -> Dict[str, Any]:
    ...
    if not news_items:
        return {
            ...
            "analysis": "No recent news articles found.",
    ...
    prompt = f"""Analyze sentiment of these news headlines for {ticker_clean} (NSE India stock).
...
SOURCES: [top 2 publishers]"""
    ...
    bullish_kw = ["profit","growth","buy","upgrade","outperform","record","beat","surge","rally","strong"]
    bearish_kw = ["loss","decline","sell","downgrade","underperform","miss","drop","weak","cut","fall"]
    ...
    return {
        ...
        "analysis":          f"{len(news_items)} recent articles found. Sentiment: {sentiment} based on keyword analysis.",
